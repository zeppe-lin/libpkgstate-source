...
for file in tests/contracts/check_style.sh ci/lint-manpage.sh ci/build-dependencies.sh ci/audit-shared-boundary.sh; do [ -x "$root/$file" ] || fail "missing executable $file"; done
